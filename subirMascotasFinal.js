...
// Función para convertir enfermedades a objetos
const convertirEnfermedades = (enfermedades) => {
    if (!Array.isArray(enfermedades)) return [];
    
    // Mapeo de enfermedades del JSON a las válidas del esquema
    const mapeoEnfermedades = {
        'gripe': 'gripe',
        'Gripe': 'gripe',
        'pelos_estomago': 'pelos_estomago',
        'Pelos de estómago': 'pelos_estomago',
        'muerte': 'cancer', // Mapear muerte a cancer
        'Muerte': 'cancer'
    };
    ...
    enfermedades.forEach(enfermedad => {
        const enfermedadMapeada = mapeoEnfermedades[enfermedad];
        if (enfermedadMapeada) {
            enfermedadesValidas.push({
                tipo: enfermedadMapeada,
...
const subirMascotas = async () => {
    try {
        ...
        const mascotasParaSubir = mascotasData.map((mascota, index) => {
            ...
            // Convertir enfermedades a objetos
            const enfermedadesObjetos = convertirEnfermedades(mascota.enfermedades);
            ...
            return {
                ...
                enfermedades: enfermedadesObjetos,
        ...
        const tipos = await Mascota.aggregate([
            { $group: { _id: '$tipo', count: { $sum: 1 } } },
            { $sort: { count: -1 } }
        ]);
        tipos.forEach(tipo => {
            console.log(`   ${tipo._id}: ${tipo.count} mascotas`);
        });
